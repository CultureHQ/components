...
type VideoFieldProps = Omit<React.InputHTMLAttributes<HTMLInputElement>, HijackedProps> & {
  aspectRatio?: number;
  autoFocus?: boolean;
  ...
  progress?: number;
  required?: boolean;
  validator?: (value: VideoFieldValue) => FormFieldError;
...
class VideoField extends React.Component<VideoFieldProps & FormState, VideoFieldState> {
  ...
  handleVideoSelected = (
    video: VideoFieldValue, thumb: Blob | null, gifUrl: string | null, videoEditorOpen: boolean
  ) => {
  ...
  render() {
    ...
    const normal: any = value || (values[name] as VideoFieldValue | undefined) || null;
...
export default withForm(VideoField);
